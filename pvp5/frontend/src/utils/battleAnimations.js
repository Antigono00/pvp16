...
/**
 * Enhanced damage number display with damage type styling
 */
export const showDamageNumber = (
  ...
  try {
    ...
    console.log('showDamageNumber called with:', {
      ...
      isCritical,
      isBlocked,
      damageType // NEW: Log damage type
    });
    ...
    // Determine the class based on type, critical status, and damage type
    let className = '';
    let displayText = '';
    ...
    // NEW: Add damage type label for special types
    if (damageAmount > 0 && damageType !== 'normal') {
    ...
    // NEW: Add special effects for damage types
    ...
    } else if (damageType === 'glancing') {
      generateParticles(targetElement, 'deflect', 10);
    }
